...
#docker run -d --publish-all=true --cap-add=SYS_ADMIN --cap-add=NET_ADMIN --security-opt seccomp:unconfined --cpuset-cpus=$CPUIDS --name=redis-shard1 redis tail -f /dev/null 2>&1 >> $DFILE
while read ID; do
