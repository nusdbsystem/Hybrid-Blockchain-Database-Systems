...
for (( shardId=1; shardId<=${shards}; shardId++ )); do
	...
	IPX=$((${bootnode} + 1))
	echo "eth-node = \"http://${PREFIX}${IPX}:9000\"" > ${genesisDir}/shard_${shardId}.toml
	echo "eth-boot-signer-address = \"${shardsigner}\"" >> ${genesisDir}/shard_${shardId}.toml
	hexkey=`ssh -o StrictHostKeyChecking=no root@${PREFIX}${IPX} "jq -r '.crypto.ciphertext' <<< cat /Data/eth_${shardId}_1/keystore/UTC*"`
	...
	--rpc --rpcaddr '${PREFIX}${IPX}' --rpcport 9000 \
	--port 30303 \
