...
TSTAMP=`date +%F-%H-%M-%S`
LOGSD="logs-workload-blockchaindb-$TSTAMP"
mkdir $LOGSD
...
for TH in $nDISTRIBUTIONS; do
    ...
    sleep 6
    $bin --load-path=$loadPath --run-path=$runPath --ndrivers=$ndrivers --nthreads=$nthreads --server-addrs=${defaultAddrs} > $LOGSD/blockchaindb-workload-$TH.txt 2>&1
done
