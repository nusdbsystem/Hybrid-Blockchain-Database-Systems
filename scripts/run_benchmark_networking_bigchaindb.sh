...
for BW in $BWS; do    
    for RTT in $RTTS; do
	    ...
	    sleep 3
        timeout 600 python3 $RDIR/BigchainDB/bench.py temp/ycsb_data/workloada.dat temp/ycsb_data/run_workloada.dat $ADDRS $THREADS 2>&1 | tee $LOGS/bigchaindb-$BW-$RTT.txt
    done
done