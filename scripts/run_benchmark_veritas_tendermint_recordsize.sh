...
for TXSIZE in $TXSIZES; do
    ...
    sleep 10
    ../bin/veritas-tendermint-bench --load-path=$DEFAULT_WORKLOAD_PATH"_$TXSIZE/"$DEFAULT_WORKLOAD".dat" --run-path=$DEFAULT_WORKLOAD_PATH"_$TXSIZE/run_"$DEFAULT_WORKLOAD".dat" --ndrivers=$DRIVERS --nthreads=$THREADS --veritas-addrs=$ADDRS 2>&1 | tee $LOGS/veritas-tendermint-txsize-$TXSIZE.txt
done
