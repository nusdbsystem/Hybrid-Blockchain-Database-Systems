...
for idx in `seq 2 $END_IDX`; do
	ssh -o StrictHostKeyChecking=no root@$IPPREFIX.$idx "killall -9 mongod; cd  && rm -rf /data/db && mkdir -p /data/db"
	ssh -o StrictHostKeyChecking=no root@$IPPREFIX.$idx "nohup /mongodb-linux-x86_64-ubuntu2004-4.4.4/bin/mongod --bind_ip_all > mongodb.log 2>&1 &"
done
