...
for I in `seq 1 $N`; do
	...
	ssh -o StrictHostKeyChecking=no root@$ADDR "cd /; rm -rf veritas; mkdir -p /veritas/data; nohup /bin/veritas-tendermint --signature=node$I --parties=${NODES} --blk-size=100 --addr=:1990 --redis-addr=0.0.0.0:6379 --redis-db=0 --ledger-path=veritas$I --tendermint-socket=$TMSOCK1 --abci-socket=$TMSOCK2 > veritas-$I.log 2>&1 &"
done
